...
mod stdp { 
    ...
    impl Parser for U32 {
        ...
        fn parse<'a>(&self,input:&'a str)->Result<(&'a str, Self::Dest), ()> {
            ...
            let value= NonZeroU32::new(u32::from_str_radix(
                    &remaining[..end_idx],
...
/// Обобщенная функция дя парсеров
...
pub enum AppLogTraceKind {
    ...
    SendRequest(Box<String>),
    Check(Box<Announcements>),
    GetResponse(Box<String>),
}
        ...
        preceded(
            ...
            alt4(
                ...
                map(
                    ...
                    |trace| AppLogTraceKind::SendRequest(Box::new(trace))
                ),
                map(
                    preceded(
                ...
                map(
                    ...
                    |trace| AppLogTraceKind::GetResponse(Box::new(trace))
                ),
            )
        )
    }
...
mod test {
    ...
    #[test]
    fn test_u32() {
        assert_eq!(stdp::U32.parse("411".into()), Ok(("".into(), NonZero::new(411).unwrap())));
        assert_eq!(stdp::U32.parse("411ab".into()), Ok(("ab".into(), NonZero::new(411).unwrap())));
        assert_eq!(stdp::U32.parse("".into()), Err(()));
        assert_eq!(stdp::U32.parse("-3".into()), Err(()));
        assert_eq!(stdp::U32.parse("0x03".into()), Ok(("".into(), NonZero::new(0x3).unwrap())));
        assert_eq!(stdp::U32.parse("0x03abg".into()), Ok(("g".into(), NonZero::new(0x3ab).unwrap())));
        assert_eq!(stdp::U32.parse("0x".into()), Err(()));
    }

    #[test]
    fn test_i32() {
        assert_eq!(stdp::I32.parse("411".into()), Ok(("".into(), NonZero::new(411).unwrap())));
        assert_eq!(stdp::I32.parse("411ab".into()), Ok(("ab".into(), NonZero::new(411).unwrap())));
        assert_eq!(stdp::I32.parse("".into()), Err(()));
        assert_eq!(stdp::I32.parse("-3".into()), Ok(("".into(), NonZero::new(-3).unwrap())));
    ...
    fn test_strip_whitespace() {
        ...
        assert_eq!(strip_whitespace(stdp::U32).parse(" 42 answer".into()), Ok(("answer".into(), NonZero::new(42).unwrap())));
    }

    #[test]
    fn test_delimited() {
        assert_eq!(delimited(tag("["), stdp::U32, tag("]")).parse("[0x32]".into()), Ok(("".into(), NonZero::new(0x32).unwrap())));
        assert_eq!(delimited(tag("[".into()), stdp::U32, tag("]".into())).parse("[0x32] nice".into()), Ok((" nice".into(), NonZero::new(0x32).unwrap())));
    ...
    #[test]
    fn test_key_value() {
        assert_eq!(key_value("key", stdp::U32).parse(r#""key":32,"#.into()), Ok(("".into(), NonZero::new(32).unwrap())));
        assert_eq!(key_value("key", stdp::U32).parse(r#"key:32,"#.into()), Err(()));
        assert_eq!(key_value("key", stdp::U32).parse(r#""key":32"#.into()), Err(()));
        assert_eq!(key_value("key", stdp::U32).parse(r#" "key" : 32 , nice"#.into()), Ok(("nice".into(), NonZero::new(32).unwrap())));
    }

    #[test]
    fn test_list() {
        assert_eq!(list(stdp::U32).parse("[1,2,3,4,]".into()), Ok(("".into(), vec![1, 2, 3, 4,].iter().map(|e|{NonZero::new(*e).unwrap()}).collect())));
        assert_eq!(list(stdp::U32).parse(" [ 1 , 2 , 3 , 4 , ] nice".into()), Ok(("nice".into(), vec![1, 2, 3, 4,].iter().map(|e|{NonZero::new(*e).unwrap()}).collect())));
    ...
    #[test]
    fn test_backet() {
        assert_eq!(Backet::parser().parse(r#"Backet{"asset_id":"usd","count":42,}"#.into()), Ok(("".into(), Backet{asset_id: "usd".into(), count: NonZero::new(42).unwrap()})));
        assert_eq!(Backet::parser().parse(r#"Backet{"count":42,"asset_id":"usd",}"#.into()), Ok(("".into(), Backet{asset_id: "usd".into(), count: NonZero::new(42).unwrap()})));
    }
    ...
    fn test_log_kind() {
        ...
        assert_eq!(LogKind::parser().parse(r#"App::Journal CreateUser {"user_id": "Steeve", "authorized_capital": 10000,}"#.into()), Ok(("".into(), LogKind::App(AppLogKind::Journal(AppLogJournalKind::CreateUser{user_id: "Steeve".into(), authorized_capital: NonZero::new(10_000).unwrap()})))));
        ...
        assert_eq!(LogKind::parser().parse(r#"App::Journal RegisterAsset {"asset_id": "bayc", "liquidity": 100000000, "user_id": "Steeve",}"#.into()), Ok(("".into(), LogKind::App(AppLogKind::Journal(AppLogJournalKind::RegisterAsset{asset_id: "bayc".into(), user_id: "Steeve".into(), liquidity: NonZero::new(100_000_000).unwrap()})))));
        assert_eq!(LogKind::parser().parse(r#"App::Journal DepositCash UserCash{"user_id": "Steeve", "count": 10,}"#.into()), Ok(("".into(), LogKind::App(AppLogKind::Journal(AppLogJournalKind::DepositCash(UserCash{user_id: "Steeve".into(), count: NonZero::new(10).unwrap()}))))));
        assert_eq!(LogKind::parser().parse(r#"App::Journal BuyAsset UserBacket{"user_id": "Steeve", "backet": Backet{"asset_id":"bayc","count":1,},}"#.into()), Ok(("".into(), LogKind::App(AppLogKind::Journal(AppLogJournalKind::BuyAsset(UserBacket{user_id: "Steeve".into(), backet: Backet{asset_id: "bayc".into(),count:NonZero::new(1).unwrap()}}))))));
    }
}
